/**
 * PRODUCTION SEED SCRIPT - Final Version
 *
 * This script waits for auth to be ready
...
(async function() {
  ...
  try {
    ...
    console.log('⏳ Waiting for authentication (up to 10 seconds)...')
    ...
    const currentUser = await new Promise((resolve, reject) => {
      ...
      const checkAuth = () => {
        ...
        if (user) {
          console.log(`✅ Found authenticated user after ${attempts * 500}ms`)
          resolve(user)
        } else if (attempts >= maxAttempts) {
          reject(new Error('Authentication timeout - no user found after 10 seconds'))
        } else {
    ...
    if (error.message.includes('timeout')) {
      console.error('\n💡 TIP: Refresh the page and try again immediately after page loads')
      console.error('Make sure you see your profile photo/name in the top right')
    }

    console.error('\nTroubleshooting:')
    console.error('  - Make sure you are logged in as: joseph@crucibleanalytics.dev')
    console.error('  - Look for your name/photo in top-right corner')
    console.error('  - Try refreshing the page and running again immediately')
  }
})();
