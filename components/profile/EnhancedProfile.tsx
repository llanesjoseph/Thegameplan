 ...
 return (
  <div className="min-h-screen">
    ...
    <div className="relative max-w-6xl mx-auto px-6 py-16">
     <div className="grid lg:grid-cols-3 gap-8">
      <div className="lg:col-span-2">
       ...
       {isEditing ? (
        <input value={current.name} onChange={e => updateField('name', e.target.value)} className="text-4xl bg-black/20 p-2 rounded-lg border border-white/20 w-full mb-3" />
       ) : (
   ...
   <section className="max-w-6xl mx-auto px-6 py-10">
    <h2 className="text-2xl mb-6">Content Statistics</h2>
    <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
     <div className="bg-black/20 border border-white/10 rounded-2xl p-6 text-center">
      <div className="text-2xl">{current.contentStats.totalLessons}</div>
      <div className="text-sm text-white/70">Total Lessons</div>
     </div>
     <div className="bg-black/20 border border-white/10 rounded-2xl p-6 text-center">
      <div className="text-2xl">{current.contentStats.totalViews.toLocaleString()}</div>
      <div className="text-sm text-white/70">Total Views</div>
     </div>
     <div className="bg-black/20 border border-white/10 rounded-2xl p-6 text-center">
      <div className="text-2xl">{current.contentStats.averageRating.toFixed(1)}</div>
      <div className="text-sm text-white/70">Avg Rating</div>
     </div>
     <div className="bg-black/20 border border-white/10 rounded-2xl p-6 text-center">
      <div className="text-2xl">{Math.round(current.contentStats.totalDuration / 60)}h</div>
